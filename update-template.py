...
with open('c:\\\\Users\\\\huzai\\\\Desktop\\\\vt academy\\\\admin-script.js', 'r', encoding='utf-8') as f:
    code = f.read()

prefix_pattern = r'(html \+= `[\s\S]*?<img src="data:image/png;base64,)'
suffix_pattern = r'(" style="height: [\s\S]*?</div>\n    </div>\n</div>\n`;)'

prefix_match = re.search(prefix_pattern, code)
suffix_match = re.search(suffix_pattern, code)

if not prefix_match or not suffix_match:
    print('Could not find matches')
    exit(1)

base64_data = code[prefix_match.end():suffix_match.start()]
...
new_suffix = r'''" style="height: 55px; object-fit: contain; margin: 0 auto; display: block;" alt="Logo">
    </div>
...
final_code = code[:prefix_match.start()] + new_prefix + base64_data + new_suffix + code[suffix_match.end():]

with open('c:\\\\Users\\\\huzai\\\\Desktop\\\\vt academy\\\\admin-script.js', 'w', encoding='utf-8') as f:
    f.write(final_code)
